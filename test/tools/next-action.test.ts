...
describe("nextActionTool", () => {
  it("returns the next action for a named project", async () => {
    const result = await nextActionTool.handler({ project: "Sample Active" }, FIXTURE, DEFAULT_CONFIG);
  ...
  it("returns next actions for all active projects when no project name is provided", async () => {
    const result = await nextActionTool.handler({}, FIXTURE, DEFAULT_CONFIG);
  ...
  it("returns a clear message when the named project does not exist", async () => {
    const result = await nextActionTool.handler({ project: "Nonexistent" }, FIXTURE, DEFAULT_CONFIG);
