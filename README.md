...
| Name                  | Purpose                                                                                                |
| --------------------- | ------------------------------------------------------------------------------------------------------ |
| `find_project`        | List PARA projects in `1-Projects/` with optional filtering, sorting, and a limit. Returns `{projects, parseFailures}` — the second field is an unfilterable census of projects whose `_project.md` couldn't be read or parsed, so a broken project can't disappear from filtered views. A present-but-unreadable `_project.md` reports `hasProjectFile: true` plus `readError` rather than reporting as absent. |
